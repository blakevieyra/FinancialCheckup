...
router.get('/csv', (req, res) => {
  ...
  const user = dbGet('SELECT id, username FROM users WHERE id = ?', [req.user.id]);
...
router.get('/summary', (req, res) => {
  ...
  const user = dbGet('SELECT username FROM users WHERE id = ?', [req.user.id]);
  const snap = snapshotForUserMonth(req.user.id, month);
  res.json({ username: user?.username, ...snap });
});

/** GET /api/reports/executive-pdf?month=YYYY-MM */
router.get('/executive-pdf', (req, res) => {
  const month = req.query.month || new Date().toISOString().slice(0, 7);
  if (!/^\d{4}-\d{2}$/.test(month)) {
    return res.status(400).json({ error: 'month must be YYYY-MM.' });
  }
...
router.get('/business-docs', (req, res) => {
  const month = req.query.month || new Date().toISOString().slice(0, 7);
  if (!/^\d{4}-\d{2}$/.test(month)) {
    return res.status(400).json({ error: 'month must be YYYY-MM.' });
  }
